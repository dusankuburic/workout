...
//function to assign color recursively
bool GraphColoring::solve(size_t nodeIndex) {
	...
	for (int colorIndex = 1; colorIndex <= numOfColors; colorIndex++) {
		...
		//check if the given color is valid (no adjecent nodes can share the same color)
		if (isColorValid(nodeIndex, colorIndex)) {

			//the color is valid: assign the proceed with next vertex
	...
	//we have considerd all the colors without any result which means there is no solution
...
//function to chack if it is valid to use that color to a given vertex
