...
bool QueensProblem::setQueens(int colIndex) {
	...
	//so we have found valid palces for all the queens
	...
	for (int rowIndex = 0; rowIndex < numOfQueens; ++rowIndex) {
		...
		if (isPlaceValid(rowIndex, colIndex)) {
			...
			//what to do? keep iterating and consider the next row (maybe it is a valid place for a queen)
			chessTable[rowIndex][colIndex] = 0;
		}
	}
	
	//we have considered all the rows without any result which means there is no solution
	//we are not able to put the queens such that they can not attach each other
...
bool QueensProblem::isPlaceValid(int rowIndex, int colIndex) const {
	...
	//there is a queen in the diagonal (form top left to bottom right direction)
	for (int i = rowIndex, j = colIndex; i >= 0 && j >= 0; i--, j--) {
		if (chessTable[i][j] == 1)
			return false;
	}

	//there is queen in the diagonal (from top right to bottom left direction)
